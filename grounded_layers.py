...
class Grounded_GRU(Recurrent):
    ...
    def __init__(self, units, num_labels,
                 activation='tanh',
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(Grounded_GRU, self).__init__(**kwargs)
        self.units = units
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            recurrent_kernel[:self.num_labels, : self.num_labels] = Multiply([recurrent_kernel[:self.num_labels, : self.num_labels], eye_mask])
            recurrent_kernel[:self.num_labels, self.units : self.units + self.num_labels] = Multiply([recurrent_kernel[:self.num_labels, self.units : self.units + self.num_labels], eye_mask]) 
            recurrent_kernel[:self.num_labels, 2 * self.units: 2*self.units + self.num_labels] = Multiply([recurrent_kernel[:self.num_labels, 2 * self.units: 2*self.units + self.num_labels], eye_mask]) 
...
class Grounded_Dense(Layer):
    ...
    def __init__(self, units,
        ...
        super(Grounded_Dense, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.use_bias = use_bias
        self.kernel_initializer = initializers.get(kernel_initializer)
    ...
    def call(self, inputs):
        ...
        output = tf.log(inputs) - tf.log(1.0 - inputs)
        output = output[:, :self.units]
        # output = InverseSigmoid()(inputs)
        if self.use_bias:
